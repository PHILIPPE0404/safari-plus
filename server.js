const express = require('express');
const axios = require('axios');
const path = require('path');

const app = express();
const PORT = process.env.PORT || 3000;

app.use(express.static(path.join(__dirname, 'public')));

app.use((req, res, next) => {
  console.log(`[LOG] ${new Date().toLocaleTimeString()} - ${req.method} ${req.url}`);
  next();
});

// Utilitaire pour résoudre une URL relative en URL absolue
function resolveUrl(relative, base) {
  try {
    return new URL(relative, base).href;
  } catch (e) {
    return relative;
  }
}

// Convertit une URL cible en URL proxy
function proxyUrl(url) {
  return '/api/proxy?url=' + encodeURIComponent(url);
}

// 1. Moteur de recherche (API Wikipédia FR)
app.get('/api/search', async (req, res) => {
  const query = req.query.q;
  if (!query) return res.status(400).json({ error: 'Recherche vide' });

  try {
    const wikiRes = await axios.get('https://fr.wikipedia.org/w/api.php', {
      params: { action: 'query', list: 'search', srsearch: query, format: 'json', origin: '*' },
      headers: { 'User-Agent': 'SafariPlusApp/1.0' },
      timeout: 5000
    });

    if (wikiRes.data?.query?.search) {
      const results = wikiRes.data.query.search.slice(0, 10).map(item => ({
        title: item.title,
        link: 'https://fr.wikipedia.org/wiki/' + encodeURIComponent(item.title),
        snippet: item.snippet.replace(/<[^>]*>?/gm, '')
      }));
      return res.json({ results });
    }
  } catch (err) {
    console.error('[RECHERCHE ERROR]', err.message);
  }
  res.status(500).json({ error: 'Aucun résultat trouvé.' });
});

// Fonction principale de Proxying
async function handleProxyRequest(targetUrl, req, res) {
  try {
    const targetObj = new URL(targetUrl);

    const response = await axios({
      method: req.method,
      url: targetUrl,
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36',
        'Accept': '*/*',
        'Accept-Language': 'fr-FR,fr;q=0.9,en;q=0.8',
        'Referer': targetObj.origin + '/'
      },
      responseType: 'arraybuffer',
      validateStatus: () => true,
      timeout: 12000
    });

    const contentType = response.headers['content-type'] || '';

    // Suppression des en-têtes de sécurité distants
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Content-Type', contentType);
    res.removeHeader('X-Frame-Options');
    res.removeHeader('Content-Security-Policy');
    res.removeHeader('Content-Security-Policy-Report-Only');

    // A. Réécriture HTML + Injection JS client
    if (contentType.includes('text/html')) {
      let html = response.data.toString('utf-8');

      // Réécriture des attributs (src, href, action)
      html = html.replace(/(href|src|action)=(['"])(.*?)\2/gi, (match, attr, quote, url) => {
        if (!url || url.startsWith('data:') || url.startsWith('#') || url.startsWith('javascript:')) {
          return match;
        }
        const abs = resolveUrl(url, targetUrl);
        return `${attr}=${quote}${proxyUrl(abs)}${quote}`;
      });

      // Réécriture des images réactives (srcset)
      html = html.replace(/srcset=(['"])(.*?)\1/gi, (match, quote, srcset) => {
        const rewritten = srcset.split(',').map(part => {
          const bits = part.trim().split(/\s+/);
          if (bits[0] && !bits[0].startsWith('data:')) {
            bits[0] = proxyUrl(resolveUrl(bits[0], targetUrl));
          }
          return bits.join(' ');
        }).join(', ');
        return `srcset=${quote}${rewritten}${quote}`;
      });

      // Injection client pour intercepter Fetch, XHR et Clics
      const clientInjection = `
        <base href="${targetObj.origin}/">
        <meta name="referrer" content="no-referrer">
        <script>
          (function() {
            const BASE_TARGET = "${targetUrl}";
            function toProxy(u) {
              try {
                const abs = new URL(u, BASE_TARGET).href;
                return '/api/proxy?url=' + encodeURIComponent(abs);
              } catch(e) { return u; }
            }

            const origFetch = window.fetch;
            window.fetch = function(res, init) {
              if (typeof res === 'string' && !res.startsWith('data:') && !res.startsWith('/api/proxy')) {
                res = toProxy(res);
              }
              return origFetch.call(this, res, init);
            };

            const origXHR = window.XMLHttpRequest.prototype.open;
            window.XMLHttpRequest.prototype.open = function(m, u, ...args) {
              if (typeof u === 'string' && !u.startsWith('data:') && !u.startsWith('/api/proxy')) {
                u = toProxy(u);
              }
              return origXHR.call(this, m, u, ...args);
            };

            document.addEventListener('click', function(e) {
              const a = e.target.closest('a');
              if (a && a.href && !a.href.startsWith('javascript:') && !a.href.includes('/api/proxy')) {
                e.preventDefault();
                window.location.href = toProxy(a.href);
              }
            }, true);
          })();
        </script>
      `;

      if (html.includes('<head>')) {
        html = html.replace('<head>', '<head>' + clientInjection);
      } else {
        html = clientInjection + html;
      }

      return res.send(html);
    }

    // B. Réécriture des styles CSS (images de fond, polices)
    if (contentType.includes('text/css')) {
      let css = response.data.toString('utf-8');
      css = css.replace(/url\((['"]?)(.*?)\1\)/gi, (match, quote, url) => {
        if (!url || url.startsWith('data:')) return match;
        const abs = resolveUrl(url, targetUrl);
        return `url("${proxyUrl(abs)}")`;
      });
      return res.send(css);
    }

    // C. Rendu binaire pour images, médias et scripts
    return res.send(Buffer.from(response.data));

  } catch (err) {
    console.error('[PROXY ERROR]', err.message);
    return res.status(500).send('Erreur lors du chargement : ' + err.message);
  }
}

// 2. Route officielle Proxy
app.get('/api/proxy', async (req, res) => {
  let targetUrl = req.query.url;
  if (!targetUrl) return res.status(400).send('URL manquante');
  if (!targetUrl.startsWith('http://') && !targetUrl.startsWith('https://')) {
    targetUrl = 'https://' + targetUrl;
  }
  await handleProxyRequest(targetUrl, req, res);
});

// 3. Catch-All : Récupère les images et ressources relatives omises par le HTML
app.use(async (req, res, next) => {
  const referer = req.headers['referer'] || '';
  if (referer.includes('/api/proxy?url=')) {
    try {
      const match = referer.match(/url=([^&]+)/);
      if (match && match[1]) {
        const parentUrl = decodeURIComponent(match[1]);
        const parentObj = new URL(parentUrl);
        const missingAssetUrl = parentObj.origin + req.originalUrl;
        return await handleProxyRequest(missingAssetUrl, req, res);
      }
    } catch (e) {}
  }
  next();
});

app.listen(PORT, () => {
  console.log('[SERVEUR] Safari+ démarré sur le port ' + PORT);
});
